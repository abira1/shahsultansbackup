...
import { ListeningTrackSummary } from '../../../types/listening';

const ListeningTracks: React.FC = () => {
  const navigate = useNavigate();
  const [tracks, setTracks] = useState<ListeningTrackSummary[]>([]);
  ...
  const loadTracks = async () => {
    try {
      setLoading(true);
      const tracksData = await listeningService.getAllTracks();
  ...
  const handleDeleteTrack = async (trackId: string, trackTitle: string) => {
    ...
    try {
      await listeningService.deleteTrack(trackId);
  ...
  const handleTogglePublish = async (trackId: string, isPublished: boolean) => {
    try {
      await listeningService.togglePublish(trackId, !isPublished);
    ...
    <AdminLayout>
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Listening Tracks</h1>
            <p className="text-gray-600">Manage your IELTS listening test tracks</p>
          </div>
          <button
            onClick={() => navigate('/admin/listening/tracks/new')}
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow-sm border p-8 text-center">
            ...
            <p className="text-gray-600 mb-4">
              {tracks.length === 0 
                ? 'Create your first listening track to get started'
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-2 ml-4">
                      <button
                        onClick={() => navigate(`/admin/listening/tracks/${track.id}/sections`)}
                      ...
                      <button
                        onClick={() => navigate(`/admin/listening/tracks/${track.id}/edit`)}
...
export default ListeningTracks;